...
- **Structural docs.** `CLAUDE.md` (rules and commands), `docs/architecture.md` (the map), `docs/spec.md` (design for
  what is not built; built work collapses to a `**Built.**` note). `docs/glossary.md` starts with the generator port.
- **Implement.** The lead edits directly (ADR 0003): step 1 of `03-implement.md` does not apply, every other step
  does.
- **Checks.** `npm run typecheck` and `npm test` must pass before implementation ends. Until the generator lands,
  Vitest has no tests, and only the peer-review script's own tests run; say so rather than calling the suite green.
