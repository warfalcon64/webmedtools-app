...
| 0002 | [Worksheet rules follow the legacy generator until a guideline source is added](0002-worksheet-rules-follow-the-legacy-generator.md) | a to z in order, one target per word, fillers never hold the next target; change one only via brainstorm. |
...
| 0004 | [Next.js with static export; no server until a tool needs one](0004-next-js-static-export.md) | Pages are pre-built HTML for search and ads; no server features until a brainstorm lifts `output: "export"`. |
